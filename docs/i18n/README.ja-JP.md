...
- **既存の`.claude/`設定がある？** `.cpm-backup/`にバックアップし、新規インストール後、`settings.json`を復元します
- **既存の`process/`ディレクトリがある？** installでは一切触りません——`vc-setup`がインテリジェントにマイグレーションします
- **既存の`CLAUDE.md`がある？** `CLAUDE.md.pre-cpm`としてバックアップし、ハーネスバージョンをインストールします
...
| 機能 | code-pro-max-kit | Superpowers | GSD | gstack |
|---------|---------------------|-------------|-----|--------|
...
- 🐛 [バグを報告する](https://github.com/christerjohansson-iver/code-pro-max-kit/issues/new?template=1.bug_report.yml)
- 💡 [機能をリクエストする](https://github.com/christerjohansson-iver/code-pro-max-kit/issues/new?template=2.feature_request.yml)
- ⚡ [スキルを提出する](https://github.com/christerjohansson-iver/code-pro-max-kit/issues/new?template=3.skill_submission.yml)
- 🌐 [翻訳を追加する](https://github.com/christerjohansson-iver/code-pro-max-kit/issues/new?template=5.translation.yml)
...
違いは：code-pro-max-kit はスペック駆動の開発フレームワークと自己改善するコンテキスト組織に焦点を当て、80以上のスキルで肥大化させません。ツールは少なく、構造を重視。
